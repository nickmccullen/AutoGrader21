...
class Evaluation(Ev):
    ...
    plugin_type = 'AR10366_EX_08'
    plugin_name = 'AR10366 - Exercise 8'  
    ...
    def mark(self, execution_string, *args):        
        ...
        MODELoutput=pd.read_excel("TaskFiles/MODELTASK7-totals.xlsx")
        ...
        [os.remove(f) for f in student_files if f[-5:].lower()=='.xlsx']
        student_files = os.listdir('.')
        print(f"LS: {student_files}")
        try:
            os.mkdir("Files")
        except: pass    
        copyfile(f"{rootpath}/TaskFiles/houseenergy.csv", "Files/energy.csv")
        copyfile(f"{rootpath}/TaskFiles/houseenergy.csv", "energy.csv")
        ...
        try:
            ...
            student_files = os.listdir('.')
            print(student_files)
            textfiles = [f for f in student_files if f[-5:].lower()=='.xlsx']
            print(f"{textfiles}")
            if len(textfiles)==0:
                self._feedback_string += "Your python script did not generate any file with a .xlsx extension.\n\n"
                self._points_received -=1
            elif len(textfiles)==1:
                outputfile = textfiles[0]
                try: Soutput=pd.read_excel(outputfile)
                except: Soutput=None
                if MODELoutput!=Soutput:
                    self._feedback_string += "Your output file contains the wrong data!."
                    self._points_received -=1
                else: self._points_received=self.maxpoints
            else: 
                self._feedback_string += "Your script produces more than one excel file so cannot be checked automatically."
                self._points_received -=1
